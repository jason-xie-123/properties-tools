...
# Windows amd64
COMMAND="GOOS=windows GOARCH=amd64 go build $BUILD_PARAMETER -o $RELEASE_DIR/windows-amd64/${TARGET_NAME}.exe ./cmd/properties-cli"
echo exec: "$COMMAND"
...
# Windows 32-bit
COMMAND="GOOS=windows GOARCH=386 go build $BUILD_PARAMETER -o $RELEASE_DIR/windows-386/${TARGET_NAME}.exe ./cmd/properties-cli"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
...
# Windows ARM
COMMAND="GOOS=windows GOARCH=arm64 go build $BUILD_PARAMETER -o $RELEASE_DIR/windows-arm64/${TARGET_NAME}.exe ./cmd/properties-cli"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
...
# darwin amd64
COMMAND="GOOS=darwin GOARCH=amd64 go build $BUILD_PARAMETER -o $RELEASE_DIR/darwin-amd64/${TARGET_NAME} ./cmd/properties-cli"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
...
# darwin ARM
COMMAND="GOOS=darwin GOARCH=arm64 go build $BUILD_PARAMETER -o $RELEASE_DIR/darwin-arm64/${TARGET_NAME} ./cmd/properties-cli"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
