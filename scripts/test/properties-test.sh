...
calc_properties_cli_path() {
    ...
    if [ "$CURRENT_ARCHITECTURE" == "X64" ]; then
        if [ "$(is_windows_platform)" == "true" ]; then
            CURRENT_BINARY_PATH="$PROJECT_FOLDER/release/windows-amd64/properties-cli.exe"
        elif [ "$(is_darwin_platform)" == "true" ]; then
            CURRENT_BINARY_PATH="$PROJECT_FOLDER/release/darwin-amd64/properties-cli"
        else
            echo ""
            echo ""
        ...
        if [ "$(is_windows_platform)" == "true" ]; then
            CURRENT_BINARY_PATH="$PROJECT_FOLDER/release/windows-386/properties-cli.exe"
        else
            echo ""
            echo ""
        ...
        if [ "$(is_windows_platform)" == "true" ]; then
            CURRENT_BINARY_PATH="$PROJECT_FOLDER/release/windows-arm64/properties-cli.exe"
        elif [ "$(is_darwin_platform)" == "true" ]; then
            CURRENT_BINARY_PATH="$PROJECT_FOLDER/release/darwin-arm64/properties-cli"
        else
            echo ""
            echo ""
